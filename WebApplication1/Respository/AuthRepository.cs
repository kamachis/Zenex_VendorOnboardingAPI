...
namespace Zenex.Authentication.Respository
{
    public class AuthRepository : IAuthRepository
    {
        ...
        public AuthRepository(ZenexContext context, IConfiguration configuration)
        {
            _dbContext = context;
        ...
        public async Task<bool> VendorLogin(VendorLoginModel VendorLoginModel)
        {
            bool login = false;
            try
            {
                var vendorlogindetails = _dbContext.VendorRegisterLogin.Where(x => x.UserId == VendorLoginModel.UserId).FirstOrDefault();
                if (vendorlogindetails != null)
                {
                    string password = Encrypt(VendorLoginModel.Password, true);
                    if (vendorlogindetails.Password1 == password || vendorlogindetails.Password2 == password)
                    {
                        login =  true;
                    }
                    else
            ...
            {
                ErrorLog.WriteToFile("AuthRespository/VendorLogin/ :- ", ex);
                return false;
            }
        }
