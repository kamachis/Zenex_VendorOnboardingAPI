...
namespace Zenex.Registration.Respository
{
    public class ServiceRepository : IServiceRepository
    {
        ...
        public ServiceRepository(ZenexContext dbContext, IConfiguration configuration)
        {
        ...
        public async Task SendReminderToInitializedVendor()
        {
            try
            {
                ...
                {
                    var tokenhistory = _dbContext.RegisterTokenHistories.Where(x => x.TransID == vendor.TransID).FirstOrDefault();
                    if (tokenhistory != null)
                    {
                        if (tokenhistory.ExpireOn >= Today)
                        {
                            var tokenTimespan = (tokenhistory.ExpireOn - Today).TotalDays;
                            // bool sendresult = await SendMail(HttpUtility.UrlEncode(tokenhistory.Token), vendor.Name, vendor.Primarymail, vendor.TransID.ToString(), PortalAddress, "Vendor Registration - Reminder", tokenTimespan.ToString());
                        }
                    }
                }
            }
