...
using UserRoleMap = Zenex.DBContext.UserRoleMap;
using static Zenex.Registration.Models.Registeration;

namespace Zenex.Authentication.Respository
{
    public class MasterRepository : IMasterRepository
    {
       
            readonly ZenexContext _dbContext;
            IConfiguration _configuration;
            private int _tokenTimespan = 0;

            public MasterRepository(ZenexContext context, IConfiguration configuration)
            {
                _dbContext = context;
            ...
            public List<User> GetApprovers()
            {
                ...
                try
                {
                    var result = (from tb in _dbContext.Users
                                  join tb1 in _dbContext.UserRoleMaps on tb.UserID equals tb1.UserID
                                  join tb2 in _dbContext.Roles on tb1.RoleID equals tb2.RoleID
                                  where tb.IsActive && tb1.IsActive && tb2.IsActive && tb2.RoleName.ToLower() == "Initiator"
                                  select tb).ToList();
                    return result;
                }
                catch (Exception ex)
                ...
                try
                {
                    ...
                    if (user1 == null)
                    {
                        ...
                        user.Role = userWithRole.Role;
                            ...
                            await _dbContext.SaveChangesAsync();
                        var roles = _dbContext.Roles.Where(x => x.RoleName == userWithRole.Role).FirstOrDefault();
                        if (roles != null)
                        {
                        ...
                        else if( user2.Email == userWithRole.Email)
                        {
                            throw new ArgumentException("User with the same email address already exists");
                        }
                        
                    }
                    else
                    {
                    throw new ArgumentException("User with same name already exist");
                    //return userResult;
        ...
        public async Task<UserWithRole> UpdateUser(UserWithRole userWithRole)
        {
            ...
            //{
                ...
                try
                {
                    ...
                    User user1 = (from tb1 in _dbContext.Users
                                  where tb1.UserName == userWithRole.UserName  && tb1.UserID != userWithRole.UserID
                                  select tb1).FirstOrDefault();

                    if (user1 == null)
                    {
                        User user2 = (from tb1 in _dbContext.Users
                                      where tb1.Email == userWithRole.Email  && tb1.UserID != userWithRole.UserID
                                      select tb1).FirstOrDefault();
                        if (user2 == null)
                        {
            ...
            public async Task<UserWithRole> CreateApproverUser(Models.AuthMaster.ApproverUser approverUser)
            {
                using (var transaction = _dbContext.Database.BeginTransaction())
                {
                    UserWithRole userResult = new UserWithRole();
                    try
                    {
                        ...
                        if (user1 == null)
                        {
                            ...
                            var VendorRoleID = _dbContext.Roles.Where(x => x.RoleName.ToLower() == "Initiator").Select(y => y.RoleID).FirstOrDefault();
        ...
        public async Task<RoleWithApp> DeleteRole(RoleWithApp roleWithApp)
            ...
            public async Task<User> SendResetLinkToMail(EmailModel emailModel)
            {
            Models.AuthMaster.TokenHistory tokenHistoryResult = new Models.AuthMaster.TokenHistory();
                try
                {
                    ...
                    return  user;
                        ...
                        //    try
                        //    {
                        //    Models.AuthMaster.TokenHistory history1 = (from tb in _dbContext.TokenHistories
                        //                                 where tb.UserID == user.UserID && !tb.IsUsed
                        //                                 select tb).FirstOrDefault();
                        //        if (history1 == null)
                        //        {
                        //        Models.AuthMaster.TokenHistory history = new Models.AuthMaster.TokenHistory()
                        //            {
                        //                UserID = user.UserID,
                        //                Token = code,
        ...
        public async Task<bool> changepassword(ForgotPasswordModel forgotpassword)
        {
            bool checkpassword = false;
            User Usermaillist = new User();
            Usermaillist = _dbContext.Users.Where(x => x.Email == forgotpassword.MailID).FirstOrDefault();
            if (Usermaillist != null)
            {
                //string Encryptedoldpassword = Encrypt(ForgotPassword.OldPassword, true);
                //if (Usermaillist.Password == forgotpassword.OldPassword)
                //{
                    string Encryptednewpassword = Encrypt(forgotpassword.NewPassword, true);
                    Usermaillist.Password = Encryptednewpassword;
                    checkpassword = true;
                    _dbContext.SaveChanges();
               // }
                //else
            ...
            public async Task<Models.AuthMaster.TokenHistory> ForgotPassword(ForgotPassword forgotPassword)
            {
                ...
                try
                {
                    ...
                    if (decryptedArray.Length == 3)
                    {
                        ...
                        if (user.UserName == decryptedArray[1] && forgotPassword.UserID == user.UserID)
                        {
                            ...
                            Models.AuthMaster.TokenHistory history = _dbContext.TokenHistories.Where(x => x.UserID == user.UserID && !x.IsUsed && x.Token == forgotPassword.Token).Select(r => r).FirstOrDefault();
                                if (history != null)
                                {
                                    var index = user.LastChangedPassword;
                                    var lastchangedIndex = 0;
