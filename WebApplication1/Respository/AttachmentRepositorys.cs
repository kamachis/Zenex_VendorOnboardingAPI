...
namespace Zenex.Registration.Respository
{
    public class AttachmentRepository : IAttachmentRepository
    {
        ...
        public AttachmentRepository(ZenexContext dbContext)
        {
        ...
        //    }
        //}
        public async Task<IdentityAttachments> UploadUserAttachment(IdentityAttachments BPAttachment)
        {
            try
            {
                ...
                //    {
                var bPAttachment = _dbContext.IdentityAttachments.Where(x => x.TransID == BPAttachment.TransID).ToList();
                        if (bPAttachment != null)
                        {
                    foreach(var Attachment in bPAttachment)
                    {
                        if(Attachment.DocType == BPAttachment.DocType)
                        {
                            Attachment.AttachmentFile = BPAttachment.AttachmentFile;
                            Attachment.AttachmentName = BPAttachment.AttachmentName;
                            Attachment.ContentType = BPAttachment.ContentType;
                            Attachment.ContentLength = BPAttachment.ContentLength;
                            Attachment.Size = BPAttachment.Size;
                            Attachment.Date = BPAttachment.Date;
                            Attachment.IDNumber = BPAttachment.IDNumber;
                            await _dbContext.SaveChangesAsync();
                            return Attachment;
                        }
                    }
